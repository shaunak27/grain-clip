...
def test(val_loader, model, backbone, device,vondrick_desc_file=None,use_desc=True):
    ...
    for class_idx, classname in enumerate(list_of_classes):
        if use_desc:
            ...
            if '/' in classname:
                print(classname)
                listy = classname.split('/')
                cls_fg = listy[-1]
                if len(listy[-2]) > 1:
                    templates_new = [t + f" of a {listy[-2]}" for t in templates]
                texty = [t.format(cls_fg) for t in templates_new]
            else:
...
def main(args):
    ...
    #args.datasets = ['imagenet','cub','places365','food101','oxfordiiitpets','dtd','cifar10','cifar100','sun397','stanfordcars','caltech101','flowers102']
    args.datasets = ['cifar10','cifar100','sun397','stanfordcars','dtd','oxfordiiitpets','caltech101','flowers102','cub','places365','food101','imagenet']
    test_datasets,vondrick_paths = get_data_loader(args,tfms)
    test_dataset_ordered = {k: test_datasets[k] for k in args.datasets if k in test_datasets}
    vondrick_paths = {k: vondrick_paths[k] for k in args.datasets if k in vondrick_paths} 
    ...
    for dataset in test_datasets:
        ...
        if dataset != 'imagenet':
            average += test_acc['acc1']    

    if distributed_utils.is_main_process():
        print('Testing complete')
        print("Average Accuracy: ",average/(len(args.datasets)-1))
        json.dump(acc_dump,open('zero_shot_results_vondrick_cc3m.json','w'))
...
def parse_args():
    ...
    parser.add_argument('--vondrick_root', default='#/classify_by_description_release/descriptors/', type=str)
