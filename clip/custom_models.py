...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, desc, return_hidden_state=True,use_desc_train=False):
        if return_hidden_state:
            image_embed = self.visual(image, return_hidden_state=True)
        else:
            image_embed = self.encode_image(image)
        text_embed = self.encode_text(text)
        return_dict =  {'image_embed': image_embed,
                'text_embed': text_embed,
                'logit_scale': self.logit_scale.exp()}
        if use_desc_train:
            desc_embed = self.encode_text(desc)
            return_dict['desc_embed'] = desc_embed
        return return_dict
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward(self, x: torch.Tensor,return_hidden_state=False) -> torch.Tensor:
...
def CLIP_VITL14(**kwargs):
    vision_model = vit_large_patch14_224(qk_norm=True, num_classes=0)
