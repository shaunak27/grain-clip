...
sys.path.insert(0, "../")
from model.box_ops import box_cxcywh_to_xyxy, generalized_box_iou
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, exclude_class=False):
        ...
        #print(len(tgt_bbox),len(out_bbox))
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, box_type, exclude_class=False):
        ...
        # print("num boxes before",num_boxes)
        # if is_dist_avail_and_initialized():
        #     torch.distributed.all_reduce(num_boxes)
        # print("num boxes after reduce",num_boxes)
        # num_boxes = torch.clamp(num_boxes / get_world_size(), min=1).item()
        # print("num boxes after clamp",num_boxes)
...
def prepare_targets(boxes, classes, image_size, center_crop = True):
    ...
    assert classes.shape[0] == boxes.shape[0], f"{classes.shape} and  {boxes.shape}"
    for idx in range(classes.shape[0]):
        c_ = classes[idx]
        b_ = boxes[idx]
        avail_idx = (c_ != -1) * (b_[:,2]>b_[:,0]) * (b_[:,3]>b_[:,1]) #pay attention to this! should it be c_ != -1 or c_ != 0?
...
def compute_box_loss( box_type, criterion, detr_out, target_boxes, 
                      target_classes, all_image_size, n_queries=10,print_idx=False):
    ...
    return sum(v for k,v in detr_loss_dict.items() if k in weight_dict) / (len(weight_dict) / 3)

def get_matched_indices(box_type, criterion, detr_out, target_boxes, target_classes, all_image_size, n_queries=10):
    targets = prepare_targets(target_boxes, target_classes, all_image_size, center_crop=False)
    detr_pred = detr_out
    with torch.no_grad():
        _, matched_indices = criterion(detr_pred, targets, box_type, exclude_class=True)
